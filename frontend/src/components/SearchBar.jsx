    ...
    <>
      <div className="flex max-md:flex-col justify-center items-center w-full max-w-3xl gap-2 m-auto">
        <input
          ...
          placeholder="Search"
        />
        <button className="bg-[#4C1A84] md:w-1/5 text-white rounded hover:bg-[#d60b8c] duration-200 px-6 py-3 rounded-r-md">
          Search
        </button>
      </div>
          ...
          {searchResults == 0 ? (
            ...
            searchResults.map((result) => (
              <Link to={`post/${result.id}`} key={result.id}>
                <li className="border-b font-medium text-xl p-1 ">
